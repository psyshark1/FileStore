...
public class ModelUserData{
    ...
    public UserData resetPass (String login) throws SQLException, MessagingException {
        ...
        ResultSet recordset = db.Recordset(
                "SELECT mail FROM OPENQUERY(ADSI, 'SELECT mail FROM ''LDAP://DC=megafon,DC=ru'' WHERE sAMAccountName = ''" + login + "''')");
    ...
    public boolean checkLogin(String login, boolean LDAP) throws SQLException {
        ...
        db.setStatmt();
        ResultSet recordset;
        //"SELECT givenName, sn, cn, mail, sAMAccountName FROM OPENQUERY(ADSI, 'SELECT givenName, sn, cn, mail, sAMAccountName FROM ''LDAP://DC=megafon,DC=ru'' WHERE sAMAccountName = ''" + login + "''')"
        if (LDAP) {
            recordset = db.Recordset("SELECT sAMAccountName FROM OPENQUERY(ADSI, 'SELECT sAMAccountName " +
                    "FROM ''LDAP://DC=megafon,DC=ru'' WHERE sAMAccountName = ''" + login + "''')");
            //sql = "SELECT sAMAccountName FROM OPENQUERY(ADSI, 'SELECT sAMAccountName " +
                    //"FROM ''LDAP://DC=megafon,DC=ru'' WHERE sAMAccountName = ''" + login + "''')";
        }else{
